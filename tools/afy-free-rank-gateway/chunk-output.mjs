...
const chunkSize = Math.max(1, Number(process.argv[4] || 50));
const data = JSON.parse(fs.readFileSync(input, 'utf8'));
if (!Array.isArray(data.candidates)) throw new Error('candidates missing');
fs.rmSync(outDir, { recursive: true, force: true });
fs.mkdirSync(outDir, { recursive: true });
const fields = ['countyIndex','fips','state','sourceCounty','sourcePopulation','businessName','placeId','googleMapsUri','countyRank','countyResultCount','city','liveAddress','businessStatus','sourceMode','observedAt'];
const chunks = [];
